...
export default function Visualization4() {
  ...
  const handleChangeDataset = (event) => {
    ...
    axios.get('api/threshold/cluster', { params: { dataset: event.target.value } })
      .then((response) => {
        let values = response.data.map(x=>+x).sort((a, b) => { return a - b; });
        setSelectedCluster(new Set([values[1], values[2]]))
  ...
  const fetchImages = () => {
    ...
    selected_cluster.forEach((single_cluster) => {
      ...
      let params = {
        ...
        cluster: JSON.stringify(single_cluster),
  ...
  const handleSliderCluster = (event) => {
    let updated_cluster = selected_cluster
    let value = parseInt(event.target.name)
    if(event.target.checked){
      updated_cluster.add(value)
    } else {
      updated_cluster.delete(value)
    }
    setSelectedData([]);
    setSelectedCluster(new Set(updated_cluster));
  }
        ...
        control_builder.push(
          <div key="slider-cluster">
            <FormControl component="fieldset">
              <FormLabel component="legend">Cluster Count <Icon style={{ fontSize: 20 }}>scatter_plot</Icon></FormLabel>
              <FormGroup row>
              {
                options_cluster.map((item, idx) => {
                  return <FormControlLabel key={idx} control={<Checkbox checked={selected_cluster.has(item)} onChange={handleSliderCluster.bind(this)} name={item+""} />} label={item} />;
                })
              }
              </FormGroup>
            </FormControl>
